...
readGrain = EFn.mkEffectFn3 \proxy key store -> do
  { valueRef } <- EFn.runEffectFn3 lookupPart proxy key store
...
subscribeGrain = EFn.mkEffectFn4 \proxy key listener store -> do
  { emitter } <- EFn.runEffectFn3 lookupPart proxy key store
...
unsubscribeGrain
  :: forall p a
   . Grain p a
  => EFn.EffectFn4 (p a) String (Effect Unit) Store Unit
unsubscribeGrain = EFn.mkEffectFn4 \proxy key listener store -> do
  { emitter } <- EFn.runEffectFn3 lookupPart proxy key store
...
updateGrain = EFn.mkEffectFn4 \proxy key f store -> do
  { emitter, valueRef } <- EFn.runEffectFn3 lookupPart proxy key store
...
lookupPart = EFn.mkEffectFn3 \proxy key store -> do
  partsRef <- EFn.runEffectFn2 lookupPartsRef proxy store
  mPart <- EFn.runEffectFn2 MO.get key partsRef
  case mPart of
    Just part -> pure part
    Nothing -> do
      ...
      EFn.runEffectFn3 MO.set key part partsRef
      pure part
